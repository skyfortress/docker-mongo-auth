...
while [[ RET -ne 0 ]]; do
    ...
    sleep 5
    mongo --ssl --sslAllowInvalidCertificates admin --eval "help" >/dev/null 2>&1
    RET=$?
done

# Create the admin user
echo "=> Creating admin user with a password in MongoDB"
mongo --ssl --sslAllowInvalidCertificates admin --eval "db.createUser({user: '$MONGODB_ADMIN_USER', pwd: '$MONGODB_ADMIN_PASS', roles:[{role:'root',db:'admin'}]});"
...
if [ "$MONGODB_APPLICATION_DATABASE" != "admin" ]; then
    ...
    mongo --ssl --sslAllowInvalidCertificates admin -u $MONGODB_ADMIN_USER -p $MONGODB_ADMIN_PASS << EOF
